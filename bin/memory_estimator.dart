...
List<Map<String, dynamic>> _generateCombinedGrid({
  ...
  void generate(int dataIndex, int settingIndex, Map<String, dynamic> current) {
    ...
    if (dataIndex == dataParamNames.length && settingIndex == settingParamNames.length) {
      ...
      if (!combo.containsKey('n_obs') && nObsDefault != null) combo['n_obs'] = nObsDefault;
      if (!combo.containsKey('n_sp') && nSpDefault != null) combo['n_sp'] = nSpDefault;
      if (!combo.containsKey('n_variable') && nVariableDefault != null) combo['n_variable'] = nVariableDefault;
    ...
    if (dataIndex < dataParamNames.length) {
      ...
      for (var value in values) {
        final next = Map<String, dynamic>.from(current);
        next[paramName] = value == value.toInt() ? value.toInt().toString() : value.toString();
...
void main(List<String> arguments) async {
  ...
  final parser = ArgParser()
    ...
    ..addOption('project-id',
        abbr: 'p',
        mandatory: true,
        help: 'Project ID where workflow copy will be created')
    ..addOption('workflow-id',
        abbr: 'w',
        mandatory: true,
    ...
    ..addOption('step-id',
        abbr: 's',
        mandatory: true,
        help: 'Step ID to estimate RAM for')
    ..addOption('table-step-id',
        mandatory: true,
        help: 'Table step ID to inject synthetic data into')
    ..addOption('team-name',
        help: 'Team name for workflow copy (optional)')
    ..addOption('n-obs',
        defaultsTo: '500',
    ...
    ..addOption('min-ram',
        defaultsTo: '500',
        help: 'Minimum RAM to test in MB (default: 500)')
    ..addOption('max-ram',
        defaultsTo: '40000',
        help: 'Maximum RAM to test in MB (default: 40000)')
    ..addOption('threshold',
        defaultsTo: '500',
        help: 'Stop threshold in MB (default: 500)')
    ..addOption('output',
        abbr: 'o',
        help: 'Output file path to save summary table as CSV')
    ..addFlag('help',
        abbr: 'h',
        negatable: false,
  ...
  try {
    ...
    if (results['help'] as bool) {
      ...
      print('\nUsage: dart run bin/memory_estimator.dart [options] [--setting.NAME=VALUE ...]');
      print('\nOptions:');
      print(parser.usage);
      print('\nOperator Settings:');
      print('  --setting.NAME=VALUE         Set operator setting NAME to VALUE');
      print('  --setting.NAME=min:n:max     Test range from min to max with n values');
      print('                               (performs grid search for all combinations)');
      print('');
      print('  Examples:');
      print('    --setting.k_neighbors=5');
      print('    --setting.k_neighbors=5:3:15     # Tests values: 5, 10, 15');
      print('    --setting.alpha=0.1:5:0.9        # Tests 5 values from 0.1 to 0.9');
    ...
    final workflowId = results['workflow-id'] as String;
    final stepId = results['step-id'] as String;
    final tableStepId = results['table-step-id'] as String;
    ...
    if (operatorSettingRanges.isNotEmpty || dataParamRanges.isNotEmpty) {
      // Grid search mode
      final totalCombos = (dataParamRanges.values.isEmpty ? 1 : dataParamRanges.values.fold(1, (p, l) => p * l.length))
          * (operatorSettingRanges.values.isEmpty ? 1 : operatorSettingRanges.values.fold(1, (p, l) => p * l.length));
      ...
      if (dataParamRanges.containsKey('n_obs') || nObsSingle != null) allParamNames.add('n_obs');
      ...
      for (var result in results) {
        final values = allParamNames.map((name) => result[name].toString()).join(',');
    ...
    print('Usage: dart run bin/memory_estimator.dart [options]');
    print('\nOptions:');
    print(parser.usage);
  } catch (e) {
...
class MemoryEstimatorScript {
  ...
  Future<void> cleanup({String? workflowId, String? dataSchemaId}) async {
    ...
    try {
      ...
      if (workflowId != null) {
        ...
        final workflow = await tercen.ServiceFactory().workflowService.get(workflowId);
      ...
      if (dataSchemaId != null) {
        ...
        final schema = await tercen.ServiceFactory().tableSchemaService.get(dataSchemaId);
        await tercen.ServiceFactory().persistentService.delete(schema.id, schema.rev);
      }
      ...
      for (var file in tempFiles) {
        ...
        await tercen.ServiceFactory().persistentService.delete(file.id, file.rev);
      }
  ...
  Future<double> estimateRequiredRam({
    ...
    while (ram < maxRamMb * mb) {
      ...
      if (stepState is sci.FailedState) {
        ...
        if (isMemoryError) {
          ...
          throw Exception("Step failed with non-memory error: ${stepState.error} - ${stepState.reason}");
        }
    ...
    logIndent("Range: ${(lowerBound / mb).toStringAsFixed(2)} MB - ${(upperBound / mb).toStringAsFixed(2)} MB");
    ...
    while (true) {
      ...
      if (delta < stopThreshold) {
        logIndent("Converged! Delta: ${(delta / mb).toStringAsFixed(2)} MB < ${thresholdMb} MB");
        break;
      }
  ...
  Future<sci.Workflow> _updateCubeQueryTask({
    ...
    if (step.model.axis.xyAxis.first.xAxis.graphicalFactor.factor.name == "observation") {
